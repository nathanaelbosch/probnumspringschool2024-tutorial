...
# ╔═╡ 133d1b04-62a4-4566-a06b-99dfe370effc
m0, p

# ╔═╡ a580f599-507f-488f-81c3-52e2f74711de
eltype(p).(m0)
...
begin
	...
	_tspan = (0, 15)
	_ts = range(_tspan..., length=1000)
	_ms, _Cs = ode_filter(f, y0, _ts, m0, C0, A, Q, E0, E1, p=p_true) 
	@assert length(_ms) > 1 "Implementation missing above"
	_ys = map(m -> E0 * m, _ms)
	# where we expliclty pass the true parameters
	...
	plot(layout=(4,1), xlabel=["" "" "" "t"], ylabel=labels)
	plot!(_ts, stack(_ys)', color="black", linestyle=:dash, label="")
	pinf_colors = findfirst.(eachrow(Bool.(_H)))'
	scatter!([_tspan[end]], replace(_H' * data, 0.0 => missing)', 
			 label="", color=[1 2 3 4])
end
...
!!! warning "Task"
	Implement a negative marginal log-likelihood function `nll` that computes the negative log-likelihood of the observation $u$ given the parameters $\theta$.
...
let
	...
	ms_opt, Cs_opt = ode_filter_cal(f, y0, _ts, m0, C0, A, Q, E0, E1, p=p_opt)

	plot_marginals(_ts, ms0, Cs0; linestyle=:dash)
	plot_marginals!(_ts, ms_opt, Cs_opt)
	plot!(_ts, stack(_ys)', labels="", color="black", linestyle=:dash)
	scatter!([_tspan[end]], replace(_H' * data, 0.0 => missing)', 
